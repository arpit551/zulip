...
def get_recent_conversations_recipient_id(user_profile: UserProfile,
    ...
    if recipient_id == my_recipient_id:
        return UserProfile.objects.values_list('personal_recipient_id', flat=True).get(id=sender_id)
    return recipient_id
...
def get_recent_private_conversations(user_profile: UserProfile) -> Dict[int, Dict[str, Any]]:
    ...
    recipient_map = {}
    my_recipient_id = user_profile.personal_recipient.id
    ...
    FROM (
        ...
        (SELECT
            m.id AS message_id,
            sender_profile.personal_recipient_id AS recipient_id
        FROM
            zerver_message m
        JOIN
