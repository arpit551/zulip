...
def process_count_stat(stat: CountStat, fill_to_time: datetime, realm: Optional[Realm]=None) -> None:
    ...
    if floor_to_hour(fill_to_time) != fill_to_time:
        raise ValueError(
            "fill_to_time must be on an hour boundary: %s" % (fill_to_time,))
    ...
    if fill_state is None:
        ...
        raise AssertionError(
            "Unknown value for FillState.state: %s." % (fill_state.state,))
...
def do_aggregate_to_summary_table(stat: CountStat, end_time: datetime,
    ...
    if realm is not None:
        realm_clause = "AND zerver_realm.id = " + str(realm.id)
    else:
        realm_clause = ""
    ...
    filters = {'property': stat.property, 'end_time': end_time}
    if realm is not None:
    ...
    for realm_count_ in realm_count:
        ...
        total_value[subgroup] = total_value[subgroup] + realm_count_.value

    for subgroup, value in total_value.items():
        filters = {'property': stat.property,
                   'subgroup': subgroup, 'end_time': end_time}

        if InstallationCount.objects.filter(**filters).exists():
            installation_count = InstallationCount.objects.filter(**filters).first()
            installation_count.value = installation_count.value + value
            installation_count.save()
        else:
            filters['value'] = value
            InstallationCount.objects.create(**filters)
...
def do_pull_by_sql_query(property: str, start_time: datetime, end_time: datetime, query: str,
                         group_by: Optional[Tuple[models.Model, str]], realm: Optional[Realm] = None) -> int:
    if group_by is None:
        subgroup = 'NULL'
        group_by_clause = ''
    else:
...
def sql_data_collector(output_table: Type[BaseCount], query: str,
    ...
    def pull_function(property: str, start_time: datetime, end_time: datetime,
        ...
        return do_pull_by_sql_query(property, start_time, end_time, query, group_by, realm)
...
def do_pull_minutes_active(property: str, start_time: datetime, end_time: datetime,
    ...
    for user_id, realm_id, interval_start, interval_end in user_activity_intervals:
        if realm is not None and realm.id is not realm_id:
            continue
...
def count_message_by_stream_query(realm: Optional[Realm]) -> str:
    if realm is None:
        ...
        realm_clause = "zerver_stream.realm_id = %%(realm_id)s AND"
    return """
...
def check_realmauditlog_by_user_query(realm: Optional[Realm]) -> str:
    if realm is None:
        ...
        realm_clause = "realm_id = %%(realm_id)s AND"
    return """
    INSERT INTO analytics_usercount
        (user_id, realm_id, value, property, subgroup, end_time)
    SELECT
...
def get_count_stats_(realm: Optional[Realm]=None) -> List[CountStat]:
...
def get_count_stats(realm: Optional[Realm]=None) -> Dict[str, CountStat]:
    count_stats_ = get_count_stats_(realm)
